...
func test_when_game_end_ball_stops():
	for i in range(5):
		_p1_score()
	assert_eq(_game.get_ball().get_speed(), 0)

func test_new_game_button_appears():
	for i in range(5):
		_p2_score()
	assert_true(_game.get_node("NewGame").visible)

func test_new_game_score_resets():
	for i in range(3):
		_p1_score()
	for i in range(5):
		_p2_score()
	_game.new_game()
	assert_eq(_game.get_p1_score(), 0, 'p1 score')
	assert_eq(_game.get_p2_score(), 0, 'p2 score')
	assert_eq(_game.get_node('P1Score').get_text(), '0', 'p1 label')
	assert_eq(_game.get_node('P2Score').get_text(), '0', 'p2 label')

func test_new_game_sets_ball_speed():
	for i in range(5):
		_p1_score()
	_game.new_game()
	assert_ne(_game.get_ball().get_speed(), 0)
